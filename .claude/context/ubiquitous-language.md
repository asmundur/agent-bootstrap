...
| Adoption conflict | pre-scaffold conflict, legacy collision, overwrite conflict | A pre-existing non-scaffold-managed file that collides with a scaffold target and is temporarily recorded in `.agent-scaffold.json` under `adoptionConflicts`. | `.claude/plans/preserve-pre-scaffold-artifacts.md`; `scripts/scaffold.sh` |
...
| Resolve-adopted-artifacts skill | adoption-resolution workflow, preserved-artifact resolver | The `/resolve-adopted-artifacts` skill — reviews preserved `*.pre-scaffold.*` backups, extracts template-worthy value, archives the legacy artifacts locally, and clears temporary adoption-conflict state. | `bootstrap-templates/templates/universal/skills/resolve-adopted-artifacts.md.tmpl` |
| Audit Beads Quality skill | fabricate beads history, task history fabrication | The `/audit-beads-quality` skill — audits Beads task history, improves useful sparse issues, creates missing durable work items, and deletes confirmed noise after a dry run. | `bootstrap-templates/templates/universal/skills/audit-beads-quality.md.tmpl` |
