...
You are running the sync-bootstrap skill for **agent-bootstrap**. This skill compares your local scaffold against the upstream bootstrap template and lets you selectively import improvements.

## When to Use This

Run this after:
- The bootstrap template has been updated (e.g., another project's retro added new patterns)
- You want to pull in best practices discovered by other projects using this scaffold

## Phase 1 — Locate Template Source

The bootstrap template lives at the path recorded in `.agent-scaffold.json` under `templateSource`.

Read `.agent-scaffold.json` to find `templateSource`.

## Phase 2 — Compare Files

Read `.agent-scaffold.json` and use its `files` list as the local source of truth for scaffold-managed files.

Then compare two things:
1. Every scaffold-managed file already recorded in the scaffold state
2. Any new upstream template files that are not recorded in the scaffold state yet, including new skills and Codex mirrors
...
1. Replace any `{{PLACEHOLDER}}` values in the imported content with the project-specific values from `.agent-scaffold.json`
